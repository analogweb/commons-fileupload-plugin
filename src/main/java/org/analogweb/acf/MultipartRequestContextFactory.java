 ...
 * {@link MultipartHttpServletRequest}にラップする{@link RequestContextFactory} の実装です。<br/>
 * リクエストがマルチパート形式であるかは
 * {@link ServletFileUpload#isMultipartContent(HttpServletRequest)} により判別されます。
 * 
 * @author snowgoose
 */
public class MultipartRequestContextFactory implements RequestContextFactory {
	...
	private static final Log log = Logs
			.getLog(MultipartRequestContextFactory.class);
    ...
    public RequestContext createRequestContext(ServletContext context,
		...
		if (ServletFileUpload.isMultipartContent(request)) {
			...
			log.log(PLUGIN_MESSAGE_RESOURCE, "DACF000002",
					new Object[] { fileUpload });
			String encoding = resolveEncoding(request);
			log.log(PLUGIN_MESSAGE_RESOURCE, "DACF000003",
					new Object[] { encoding });
			try {
				MultipartParameters parameters = createMultipartRequestParameters(
						request, createRequestContext(request), fileUpload,
						encoding);
				request = new DefaultMultipartHttpServletRequest(request,
						parameters);
			} catch (FileUploadException e) {
				throw new FileUploadFailureException(e);
			}
		}
		log.log(PLUGIN_MESSAGE_RESOURCE, "DACF000004", new Object[] { request
	...
	protected MultipartParameters createMultipartRequestParameters(
			...
			org.apache.commons.fileupload.RequestContext context,
			FileUpload fileUpload, String resolvedEncoding)
	...
	private MultipartRequestParameters extractFileItems(
			List<FileItem> fileItems, String encoding)
			throws FileUploadException {
			...
			if (fileItem.isFormField()) {
				try {
					...
					if (parameters.containsKey(fieldName)) {
						parameters.put(
								fieldName,
								ArrayUtils.add(String.class, param,
				...
				if (files.containsKey(fieldName)) {
					files.put(fieldName, ArrayUtils.add(MultipartFile.class,
							new FileItemMultipartFile(fileItem),
							files.get(fieldName)));
				} else {
					files.put(fieldName, ArrayUtils
							.newArray(new FileItemMultipartFile(fileItem)));
				}
			}
		}
	...
	public void setFileUploadFactory(
			FileUploadFactory<? extends FileUpload> fileUploadFactory) {
